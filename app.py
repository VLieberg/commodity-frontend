...
# Sidebar:
st.sidebar.write('<style>div.css-1adrfps{padding-top:0.5rem;}</style>', unsafe_allow_html=True)
...
col1, col2, col3 = st.sidebar.columns([1.3, 5, 0.2])
col2.metric(label=f"Predicted Price at {x}", value=f'${round(resposta[x],2)}', delta=(f'{round((predict_price-1691.00)/1691,2)}%, {round((predict_price-1691.00),2)}$'))
